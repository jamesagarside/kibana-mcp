...
async def _call_suspend_process(
    ...
    Args:
        ...
        endpoint_ids: List of endpoint IDs to suspend processes on
        parameters: Parameters for the suspend process action, must include at least a process ID/name
        agent_type: The type of agent (default: "endpoint")
        comment: Optional comment to include with the action

    Returns:
        JSON string with the response from the suspend process action
    """
    ...
    tool_logger.info(
        f"Suspending process {process_id or process_name} on {len(endpoint_ids)} endpoints with comment: {comment}")
    ...
    try:
        ...
        formatted_response = {
            ...
            "status": "Process suspension initiated",
            "message": f"Process suspension started on {len(endpoint_ids)} endpoint(s)"
        }

        return json.dumps(formatted_response, indent=2)

    except httpx.HTTPError as e:
        tool_logger.error(f"Error suspending process on endpoints: {e}")
        return json.dumps({
            "error": str(e),
            "status": "failed",
            "message": f"Failed to suspend process on {len(endpoint_ids)} endpoint(s)",
            "process": process_id or process_name
        }, indent=2)
