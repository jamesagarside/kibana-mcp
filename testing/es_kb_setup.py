...
def create_index_template(es_base_url, es_auth):
        ...
        "template": {
            ...
            "mappings": {
                "properties": {
                    ...
                    "message": { "type": "text" } # Keep message as text if present
    ...
    headers = {"Content-Type": "application/json"}
    try:
        response = requests.put(url, auth=es_auth, headers=headers, json=template_payload, verify=False, timeout=10)
        if 200 <= response.status_code < 300:
            print_info(f"Index template '{template_name}' created/updated successfully.")
            return True
        else:
            print_warning(f"Failed to create/update index template '{template_name}' (HTTP {response.status_code}): {response.text}")
            return False
    except requests.exceptions.RequestException as e:
        print_error(f"Error creating/updating index template '{template_name}': {e}")
        return False 