...
MAX_ALERT_WAIT_SECONDS = 150  # Wait up to 2.5 minutes for alerts (increased from 90)
...
def create_sample_alert_rule(kibana_base_url, kibana_auth):
    """Creates a sample DETECTION rule using the Kibana API. Returns the rule name on success, None otherwise."""
    ...
    try:
        ...
        if 200 <= response.status_code < 300:
            ...
            # This requires a separate API call to GET /rules/_find
            ...
            find_params = {"filter": f'alert.attributes.name: "{rule_name}"'}
            try:
...
def wait_for_signals(kibana_base_url, kibana_auth, rule_id):
    ...
    # Payload for signal search API (POST request)
    # Filter by the rule ID that generated the signal
    search_payload = {
        "query": {
            "bool": {
                "filter": [
    ...
    while time.time() - start_time < MAX_ALERT_WAIT_SECONDS:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Check hits.total.value for Elasticsearch 7.0+
            ...
            # Signals endpoint might return 404 if the internal signals index doesn't exist yet
...
def main():
    ...
    # Use the admin user ('elastic') for all script API calls
    kibana_api_auth = es_auth # Use elastic user for API operations
    # kibana_internal_auth = (KIBANA_SYSTEM_USER, KIBANA_SYSTEM_PASSWORD) # For Kibana's internal use
    ...
    # 5. Setup Kibana User and Role (for Kibana internal use)
    ...
    if wait_for_kibana(kibana_base_url, kibana_api_auth): # Check API status with elastic user
        ...
        rule_id = create_sample_alert_rule(kibana_base_url, kibana_api_auth)
        ...
        if rule_id:
            ...
            # --- Wait for signals THIRD (using elastic user) ---
        ...
        print_warning("Could not confirm Kibana status, skipping sample rule creation and signal verification.")
    ...
    print(f"      Kibana internal user setup status: {pw_status}")
    print(f"      Trigger document write status: {trig_status}")
    if signals_verified:
        print("      Successfully verified that detection signals were generated.")
    else:
        print("      WARNING: Could not verify detection signals within time limit.") # Simplified
